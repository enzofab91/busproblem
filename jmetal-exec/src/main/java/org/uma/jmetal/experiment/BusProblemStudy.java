...
public class BusProblemStudy  {
  ...
  static List<TaggedAlgorithm<List<BusSolution>>> configureAlgorithmList(
      List<Problem<BusSolution>> problemList, int independentRuns) {
    ...
    for (int run = 0; run < independentRuns; run++) {
     ...
     /* for (int i = 0; i < problemList.size(); i++) {
      ...
      for (int i = 0; i < problemList.size(); i++) {
        Algorithm<List<BusSolution>> algorithm = new NSGAIIBuilder<BusSolution>(problemList.get(i), new BusProblemCrossover(0.60),
            new BusProblemMutation(0.01))
        	.setSelectionOperator(new BinaryTournamentSelection<BusSolution>( new RankingAndCrowdingDistanceComparator<BusSolution>()))
            .setMaxEvaluations(25000)
            .setPopulationSize(100)
